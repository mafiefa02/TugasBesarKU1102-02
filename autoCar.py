...
if __name__ == "__main__":
    ...
    while True:
        ...
        if car_can_move == False:
            ...
            pos_matrix = planeGenerator.update_road(pos_matrix, lebar_jalan, tinggi_jalan)
            planeGenerator.generate_road(pos_matrix, lebar_jalan, tinggi_jalan)             # print jalan
            time.sleep(1)

        if car_can_move:
            mobilEngine.placeCar(pos_matrix, 'up', car_position)
            pos_matrix = planeGenerator.update_road(pos_matrix, lebar_jalan, tinggi_jalan)  # remove baris paling bawah dari jalan
            planeGenerator.generate_road(pos_matrix, lebar_jalan, tinggi_jalan)             # print jalan
            time.sleep(1)
